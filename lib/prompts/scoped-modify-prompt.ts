...
7. Never import npm packages other than "react", "react-dom", "framer-motion", "lucide-react", "react-router-dom", "@tanstack/react-query", "zustand", "react-hook-form", "zod", "@hookform/resolvers", "date-fns", "react-markdown", "@dnd-kit/core", "@dnd-kit/sortable", "@dnd-kit/utilities", and "recharts" — only those and relative paths are installed; anything else breaks the build. "@supabase/supabase-js" is one exception: allowed ONLY if this message contains a "SUPABASE BACKEND CONNECTED" block, and only via import { supabase } from './lib/supabase' — never call createClient() yourself or invent a URL/key. "@stripe/stripe-js" and "@stripe/react-stripe-js" are the other exception: allowed ONLY if this message contains a "STRIPE PAYMENTS CONNECTED" block, and only via import { stripePromise } from './lib/stripe' — never call loadStripe() yourself or invent a publishable key, and never fake a successful charge in the submit handler.
8. Keep every data-aiwp-slot attribute exactly as it is (invisible editor metadata). If the request asks to put an image in "spot N" or "box N", set the src (or background-image) of the element whose data-aiwp-slot equals N to the exact URL provided, keeping the existing layout classes.
